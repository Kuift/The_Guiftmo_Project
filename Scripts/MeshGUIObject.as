...
class MeshGUIObject
{
    ...
    void refresh()
    {
        v_raw[vertexArrayStartIndex  ] = (Vertex(newPosition.x - size.x, newPosition.y - size.y, 1000, _icon.getUVX(0), _icon.getUVY(0),SColor(0x70aacdff))); //upper left
		v_raw[vertexArrayStartIndex+1] = (Vertex(newPosition.x + size.x, newPosition.y - size.y, 1000, _icon.getUVX(1), _icon.getUVY(1),SColor(0x70aacdff))); //upper right
		v_raw[vertexArrayStartIndex+2] = (Vertex(newPosition.x + size.x, newPosition.y + size.y, 1000, _icon.getUVX(2), _icon.getUVY(2),SColor(0x70aacdff))); //bottom right
		v_raw[vertexArrayStartIndex+3] = (Vertex(newPosition.x - size.x, newPosition.y + size.y, 1000, _icon.getUVX(3), _icon.getUVY(3),SColor(0x70aacdff))); //bottom left
    }
